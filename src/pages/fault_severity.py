...
def show_fault_severity_page():
    ...
    try:
        handler = FaultSeverityHandler(
            electrical_model_path=MODEL_PATH, image_model_path=WEIGHTS_PATH
        )
            ...
            with st.spinner("Decoding AI logic..."):
                ...
                if not missing:
                    ...
                    non_numeric_cols = [
                        ...
                        if numeric_df[col].isnull().sum() > subset[col].isnull().sum()
                    ]

                    if not null_cols and not non_numeric_cols:
                        st.success(
                            f"File validated ({len(df)} rows). Analysis complete!"
                        )
                        ...
                        if analysis_res:
                            ...
                            if score <= 0.3:
                                status, bg_color = "Low", "#D4EDDA"
                            elif score <= 0.6:
                                status, bg_color = "Medium", "#FFF3CD"
                            elif score <= 0.8:
                                status, bg_color = "High", "#FFE5D0"
                            else:
                                status, bg_color = "Critical", "#F8D7DA"
                            ...
                            m2.metric(
                                "CONFIDENCE", f"{analysis_res.reading_confidence:.2f}%"
                            )
                        ...
                        if null_cols:
                            st.error(f"Nulls found: {', '.join(null_cols)}")
                        if non_numeric_cols:
                            st.error(
                                f"Non-numeric found: {', '.join(non_numeric_cols)}"
                            )
                else:
    ...
    with tab2:
        ...
        img_file = st.file_uploader(
            "Upload Thermal Image", type=["jpg", "png", "jpeg"], key="img_up"
        )
