...
class ScoreCAM:
    ...
    def __setup_layers(self) -> None:
        ...
        try:
            ...
            self.__logger.info(
                "ScoreCAM layer references stored. "
                "No sub-model built."
            )
    ...
    def __get_feature_map(self, img_array: np.ndarray) -> np.ndarray:
        ...
        self.__logger.warning(
            "Hook did not capture feature map. "
            "Using densenet output as fallback.")
        try:
    ...
    def generate(self, img_array: np.ndarray) -> np.ndarray:
        ...
        self.__logger.info(
            f"Feature map shape: {feature_map.shape}, "
            f"channels: {n_channels}")

        # Build normalized channel masks
        masks = np.zeros(
            (n_channels, *self.__IMAGE_SIZE), dtype=np.float32)
        ...
        for start in range(0, n_channels, self.__batch_size):
            ...
            batch = [base_img * masks[c][:, :, np.newaxis]
                     for c in range(start, end)]
            preds = self.__model.predict(
                np.array(batch, dtype=np.float32), verbose=0)
...
def _init_hotspot_localizer_corrected(self, model_path: str) -> None:
    ...
    try:
        ...
        self.__logger.error(
            f"Failed to initialize HotspotLocalizer: {e}")
...
class BoundingBoxExtractor:
    ...
    def extract(
        ...
        kernel = cv2.getStructuringElement(
            cv2.MORPH_RECT,
            (self.__morph_kernel_size, self.__morph_kernel_size))
...
class LocalizationRenderer:
    ...
    def render_overlay(
        ...
        base = (original_bgr * 255).astype(np.uint8) \
               if original_bgr.dtype != np.uint8 else original_bgr.copy()
    ...
    def render_bbox(
        ...
        """
        annotated = (original_bgr * 255).astype(np.uint8) \
...
class HotspotLocalizer:
    ...
    def __init__(
        ...
        self.__bbox_extractor = BoundingBoxExtractor(
    ...
    def localize(
        ...
        if isinstance(image_input, str):
            ...
            if original_bgr is None:
                result.error = (
                    f"Could not reload for rendering: {image_input}")
                self.__logger.error(result.error)
                return result
            ...
            if original_image_path is not None:
                ...
                if original_bgr is not None:
                    ...
                    original_bgr = cv2.cvtColor(
                        original_bgr, cv2.COLOR_RGB2BGR)
            else:
                self.__logger.warning(
        ...
        try:
            pred_score        = float(
                self.__model.predict(img_array, verbose=0)[0][0])
