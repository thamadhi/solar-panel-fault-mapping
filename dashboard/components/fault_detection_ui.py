...
from dashboard.api_client import predict_electrical, predict_image, explain_electrical
from dashboard.components.explainability import render_explainability_from_api, render_pie_chart
...
def render_csv_summary_cards(api_res, df, raw_cols):
    ...
    # Side by side layout for table + explanation
    col_list, col_exp = st.columns([1, 1], gap="medium")

    # Left side table of per-string results
    with col_list:
        ...
        # Select only what you want to show
        ...
        # Use your selectable grid/table component
    ...
    with col_exp:
        ...
        with st.container(border=True):
            ...
            # Retrieve the cached raw records (so API can build engineered features)
